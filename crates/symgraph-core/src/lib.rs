...
impl Db {
    ...
    pub fn ensure_file(&mut self, path: &str, lang: &str) -> Result<i64> {
        self.conn.execute(
            "INSERT OR IGNORE INTO files(path, lang) VALUES (?1, ?2)", params![path, lang]
        )?;
        Ok(self.conn.query_row(
            "SELECT id FROM files WHERE path=?1", params![path], |r| r.get::<_, i64>(0)
        )?)
    }
...
pub fn insert_symbol(conn: &mut Connection, file_id: i64, usr: Option<&str>, key: Option<&str>,
                     name: &str, kind: &str, is_def: bool) -> Result<i64> {
    conn.execute(
...
pub fn insert_occurrence(conn: &mut Connection, sym_id: i64, file_id: i64,
                         usage: &str, line: u32, col: u32) -> Result<i64> {
    conn.execute(
...
pub fn insert_edge(conn: &mut Connection, from_sym: Option<i64>, to_sym: Option<i64>,
                   from_module: Option<i64>, to_module: Option<i64>, kind: &str) -> Result<i64> {
    conn.execute(
...
pub fn upsert_module(conn: &mut Connection, name: &str, kind: &str, path: &str) -> Result<i64> {
    ...
    Ok(conn.query_row(
      "SELECT id FROM modules WHERE name=?1", params![name], |r| r.get::<_, i64>(0)
    )?)
}
...
mod tests {
    ...
    fn test_db_open_in_memory() {
        ...
        assert!(count >= 5, "Should have at least 5 tables (modules, files, symbols, occurrences, edges)");
    }
    ...
    fn test_insert_symbol() {
        ...
        let sym1 = insert_symbol(
            &mut db.conn, file_id, 
            Some("c:@F@main#"), None, 
            "main", "FunctionDecl", true
        ).unwrap();
        
        // Добавляем класс
        let sym2 = insert_symbol(
            &mut db.conn, file_id,
            Some("c:@S@MyClass#"), None,
            "MyClass", "ClassDecl", true
        ).unwrap();
    ...
    fn test_call_graph() {
        ...
        // Создаём функции
        let main_id = insert_symbol(&mut db.conn, file_id, Some("c:@F@main#"), None, "main", "FunctionDecl", true).unwrap();
        let foo_id = insert_symbol(&mut db.conn, file_id, Some("c:@F@foo#"), None, "foo", "FunctionDecl", true).unwrap();
        let bar_id = insert_symbol(&mut db.conn, file_id, Some("c:@F@bar#"), None, "bar", "FunctionDecl", true).unwrap();
        let baz_id = insert_symbol(&mut db.conn, file_id, Some("c:@F@baz#"), None, "baz", "FunctionDecl", true).unwrap();
        
        // main() вызывает foo() и bar()
        insert_edge(&mut db.conn, Some(main_id), Some(foo_id), None, None, "call").unwrap();
        insert_edge(&mut db.conn, Some(main_id), Some(bar_id), None, None, "call").unwrap();
    ...
    fn test_inheritance_graph() {
        ...
        // Создаём классы
        let base_id = insert_symbol(&mut db.conn, file_id, Some("c:@S@Base#"), None, "Base", "ClassDecl", true).unwrap();
        let derived_id = insert_symbol(&mut db.conn, file_id, Some("c:@S@Derived#"), None, "Derived", "ClassDecl", true).unwrap();
        let child_id = insert_symbol(&mut db.conn, file_id, Some("c:@S@Child#"), None, "Child", "ClassDecl", true).unwrap();
        
        // Derived наследует от Base
        insert_edge(&mut db.conn, Some(base_id), Some(derived_id), None, None, "inherit").unwrap();
        // Child наследует от Derived
        insert_edge(&mut db.conn, Some(derived_id), Some(child_id), None, None, "inherit").unwrap();
        
        // Проверяем: от кого наследует Base?
        let base_children = db.query_edges_by_kind_from("inherit", "c:@S@Base#").unwrap();
    ...
    fn test_member_graph() {
        ...
        // Класс Person
        let person_id = insert_symbol(&mut db.conn, file_id, Some("c:@S@Person#"), None, "Person", "ClassDecl", true).unwrap();
        
        // Поля и методы
        let name_id = insert_symbol(&mut db.conn, file_id, Some("c:@S@Person#name_"), None, "name_", "FieldDecl", true).unwrap();
        let age_id = insert_symbol(&mut db.conn, file_id, Some("c:@S@Person#age_"), None, "age_", "FieldDecl", true).unwrap();
        let get_name_id = insert_symbol(&mut db.conn, file_id, Some("c:@S@Person#getName#"), None, "getName", "Method", true).unwrap();
        
        // Связи членства
        insert_edge(&mut db.conn, Some(person_id), Some(name_id), None, None, "member").unwrap();
        insert_edge(&mut db.conn, Some(person_id), Some(age_id), None, None, "member").unwrap();
        insert_edge(&mut db.conn, Some(person_id), Some(get_name_id), None, None, "member").unwrap();
        
        // Запрос: какие члены у Person?
        let members = db.query_edges_by_kind_from("member", "c:@S@Person#").unwrap();
    ...
    fn test_module_graph() {
        ...
        insert_edge(&mut db.conn, None, None, Some(main_mod), Some(foo_mod), "module-import").unwrap();
        insert_edge(&mut db.conn, None, None, Some(main_mod), Some(bar_mod), "module-import").unwrap();
        
        // bar импортирует foo
        insert_edge(&mut db.conn, None, None, Some(bar_mod), Some(foo_mod), "module-import").unwrap();
    ...
    fn test_upsert_module_idempotent() {
        ...
        let id1 = upsert_module(&mut db.conn, "my_module", "cpp20-module", "src/my_module.cppm").unwrap();
        let id2 = upsert_module(&mut db.conn, "my_module", "cpp20-module", "src/my_module.cppm").unwrap();
        let id3 = upsert_module(&mut db.conn, "other_module", "cpp20-module", "src/other.cppm").unwrap();
