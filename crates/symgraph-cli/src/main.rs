...
fn scan_rust(manifest_path: &str, lsif: Option<&str>, db_path: &str) -> Result<()> {
    ...
    if mpath.is_dir() {
        cmd.manifest_path(mpath.join("Cargo.toml"));
    } else if mpath.is_file() {
        cmd.manifest_path(mpath);
    }
    ...
    let packages = if let Some(root) = metadata.root_package() {
        ...
        // This is a workspace - process all workspace members
...
fn process_rust_package(crate_name: &str, manifest_dir: &Path, db: &mut Db) -> Result<()> {
    ...
    // Collect symbols and calls per file via syn visitor
    #[derive(Default)]
    struct V {
        symbols: Vec<String>,
        calls: Vec<(String, String)>, // (caller, callee)
    ...
    // Walk src/ in the manifest dir for additional analysis (fallback when LSIF not provided)
        ...
        if p.is_file() && p.extension().and_then(|e| e.to_str()) == Some("rs") {
            ...
            match syn::parse_file(&s) {
                Ok(parsed) => {
                    ...
                    // Insert symbols (only if not already in DB)
                    for sym in v.symbols.iter() {
                        let fid = db.ensure_file(&path_str, "rust")?;
                        let usr = format!("r:@{}@{}", crate_name, sym);
                        // Skip if already inserted (by LSIF)
