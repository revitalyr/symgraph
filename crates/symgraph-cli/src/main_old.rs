...
use symgraph_core::{insert_edge, insert_occurrence, insert_symbol, upsert_module, Db, File, Module, Symbol, Edge};
...
enum Cmd {
    ...
    /// Analyze projects using SCIP (Source Code Intelligence Protocol).
    ///
    /// Automatically detects project language and generates SCIP index using appropriate tool:
    /// - Rust: scip-rust
    /// - C++: scip-clang (requires compile_commands.json)
    /// - Python: scip-python
    ...
    /// - Lua: scip-lua
    ///
    /// If SCIP tool is not available, provides installation instructions.
    /// For C++ projects, automatically generates compile_commands.json if needed.
    ///
    /// # Examples
    /// ```bash
    /// # Auto-detect language and scan current directory
    /// symgraph-cli scan-scip --project . --db project.db
    ///
    /// # Scan specific Rust project
    /// symgraph-cli scan-scip --project ~/my-rust-app --db rust.db
    ///
    /// # Scan C++ project with custom compile_commands.json
    /// symgraph-cli scan-scip --project ~/my-cpp-project --db cpp.db --compdb build/compile_commands.json
    ///
    /// # Scan Python project
    /// symgraph-cli scan-scip --project ~/my-python-app --db python.db
    /// ```
    ScanScip {
        /// Path to project directory to analyze.
        ///
        /// Language will be auto-detected based on project files.
        #[arg(long, value_name = "DIR", default_value = ".")]
        project: String,
        ...
        /// Path to compile_commands.json (required for C++ projects).
        ///
        /// If not provided for C++ projects, will attempt to generate automatically.
        #[arg(long, value_name = "PATH")]
        compdb: Option<String>,

        /// Force specific language instead of auto-detection.
        ///
        /// Supported: rust, cpp, python, javascript, typescript, shell, ruby, php, lua
        #[arg(long, value_name = "LANG")]
        language: Option<String>,

        /// Output path for generated SCIP file.
        ///
        /// If not specified, uses temporary file.
        #[arg(long, value_name = "PATH")]
        scip_output: Option<String>,
    },
...
fn main() -> Result<()> {
    ...
    match args.cmd {
        ...
        Cmd::ScanScip { project, db, compdb, language, scip_output } => scan_scip(&project, &db, compdb.as_deref(), language.as_deref(), scip_output.as_deref())?,
...
fn scan_cxx(compdb: &str, db_path: &str) -> Result<()> {
    ...
    // Проверяем существует ли compile_commands.json
    if !Path::new(compdb).exists() {
        bail!("compile_commands.json not found: {}. Use generate-compdb to create it first.", compdb);
    }
    ...
    for cc in cmds {
        ...
        for o in occs {
            if let Some(usr) = o.usr.as_deref() {
                if let Some(sid) = db.find_symbol_by_usr(usr)? {
                    ...
                    let _oid =
                        insert_occurrence(&mut db, &sid, &fid, &o.usage_kind, o.line, o.column)?;
                }
            }
        }
        ...
        for (kind, from, to) in edges {
            ...
            if let (Some(fs), Some(ts)) = (from_id, to_id) {
                let _eid = insert_edge(&mut db, Some(&fs), Some(&ts), None, None, &kind)?;
            }
        }
    }
    Ok(())
    ...
    for entry in WalkDir::new(root).into_iter().filter_map(|e| e.ok()) {
        ...
        if p.ends_with(".cppm") || p.ends_with(".ixx") || p.ends_with(".mxx") {
            ...
            if let Some(mi) = scan_cpp20_module(&p)? {
                ...
                let mid = upsert_module(&mut db, &mi.name, "cpp20-module", &mi.path)?;
                ...
                for imp in mi.imports {
                    ...
                    let to = upsert_module(&mut db, &imp, "cpp20-module", "")?;
...
fn scan_rust(manifest_path: &str, lsif: Option<&str>, db_path: &str) -> Result<()> {
    ...
    // Пробуем сгенерировать SCIP индекс если доступен scip-rust
    match check_scip_tool_availability(&ScipLanguage::Rust) {
        Ok(true) => {
            println!("Attempting to generate SCIP index with scip-rust...");
            let scip_output = workspace_root.join("index.scip");
            let scip_config = ScipConfig::new(ScipLanguage::Rust, workspace_root, &scip_output);
            
            match generate_scip_index(&scip_config) {
                Ok(_) => {
                    println!("SCIP index generated: {}", scip_output.display());
                    // TODO: Добавить парсинг SCIP и загрузку в базу данных
                }
                Err(e) => {
                    println!("SCIP generation failed: {}, continuing with legacy analysis", e);
                }
            }
        }
        Ok(false) => {
            println!("scip-rust not available, using legacy analysis. Install with: cargo install scip-rust");
        }
        Err(_) => {
            println!("Error checking scip-rust availability, using legacy analysis");
        }
    }
    ...
    let packages = if !metadata.workspace_members.is_empty() {
        // Use workspace members - this includes root package and all workspace crates
                            ...
                            if let Some(ref_ranges) = ref_ranges_for_result.get(&result_id) {
                                for rr in ref_ranges {
                                    if let (Some(rrange), Some(ruri)) = (ranges.get(rr), range_doc.get(rr)) {
                                        ...
                                        let _oid = insert_occurrence(db, &sid, &rfid, "reference", (rrange.start_line as u32) + 1, (rrange.start_char as u32) + 1)?;
                                    }
                                }
                            }
                        }
...
/// Lists all modules and their dependencies from the database.
fn list_modules(db_path: &str) -> Result<()> {
    ...
    for item in db.db.scan_prefix("module:") {
        ...
        if let Ok(module) = serde_json::from_slice::<crate::Module>(&value) {
    ...
    for item in db.db.scan_prefix("edge:") {
        let (_, value): (_, sled::IVec) = item?;
        if let Ok(edge) = serde_json::from_slice::<crate::Edge>(&value) {
                ...
                if let (Some(from_module), Some(to_module)) = (&edge.from_module, &edge.to_module) {
                    ...
                    if let (Some(from_data), Some(to_data)) = (
                        ...
                        if let (Ok(from_mod), Ok(to_mod)) = (
                            serde_json::from_slice::<crate::Module>(&from_data),
                            serde_json::from_slice::<crate::Module>(&to_data)
                        ) {
...
fn show_stats(db_path: &str) -> Result<()> {
    ...
    for item in db.db.scan_prefix("symbol:") {
        let (_, value): (_, sled::IVec) = item?;
        if let Ok(symbol) = serde_json::from_slice::<crate::Symbol>(&value) {
...
fn scan_scripts(root: &str, _db_path: &str) -> Result<()> {
    use anyhow::bail;
    use symgraph_discovery::{ScipConfig, ScipLanguage, detect_language, check_scip_tool_availability, get_installation_instruction};
    ...
    match detected_language {
        ScipLanguage::Python | ScipLanguage::JavaScript | ScipLanguage::TypeScript => {
            // Пробуем сгенерировать SCIP индекс
            match check_scip_tool_availability(&detected_language) {
                Ok(true) => {
                    println!("Using SCIP for {} project analysis...", detected_language);
                    
                    let scip_output = project_path.join("index.scip");
                    let scip_config = ScipConfig::new(detected_language.clone(), &project_path, &scip_output);
                    
                    match symgraph_discovery::generate_scip_index(&scip_config) {
                        Ok(_) => {
                            println!("SCIP index generated: {}", scip_output.display());
                            
                            // Парсим SCIP файл и загружаем в базу данных
                            use symgraph_core::{parse_scip_file, load_scip_to_database};
                            
                            let mut db = symgraph_core::Db::open(_db_path)?;
                            let scip_data = parse_scip_file(&scip_output)?;
                            
                            let project_name = format!("{}_project", detected_language);
                            load_scip_to_database(&mut db, &scip_data, &project_name)?;
                            
                            println!("Successfully loaded SCIP data into database: {}", _db_path);
                            println!("Loaded {} documents, {} symbols, {} occurrences", 
                                     scip_data.documents.len(), 
                                     scip_data.symbols.len(), 
                                     scip_data.occurrences.len());
                        }
                        Err(e) => {
                            println!("SCIP generation failed: {}", e);
                            let instruction = get_installation_instruction(&detected_language);
                            println!("Install SCIP tool with: {}", instruction);
                            bail!("Cannot analyze {} project without SCIP tool", detected_language);
                        }
                    }
                }
                Ok(false) => {
                    let instruction = get_installation_instruction(&detected_language);
                    println!("SCIP tool not found for {}. Install with: {}", detected_language, instruction);
                    bail!("SCIP tool required for {} analysis", detected_language);
                }
                Err(e) => {
        ...
        ScipLanguage::Unknown => {
            bail!("Cannot detect language for script project. Supported: Python, JavaScript, TypeScript");
        }
        _ => {
...
fn annotate_compiled_project(root: &str, db_path: &str) -> Result<()> {
    ...
    let files: Vec<(String, String, String)> = {
        ...
        for item in db.db.scan_prefix("file:") {
            let (_, value): (_, sled::IVec) = item?;
            if let Ok(file) = serde_json::from_slice::<crate::File>(&value) {
                files.push((
                    file.path,
...
///
/// Automatically detects project language and generates SCIP index using appropriate tool.
/// For C++ projects, can automatically generate compile_commands.json if needed.
fn scan_scip(
    project: &str,
    db_path: &str,
    compdb: Option<&str>,
    language: Option<&str>,
    scip_output: Option<&str>,
) -> Result<()> {
    use anyhow::bail;
    use symgraph_discovery::{ScipConfig, ScipLanguage, detect_language, check_scip_tool_availability, get_installation_instruction};
    use std::path::PathBuf;
    ...
    let detected_language = if let Some(lang) = language {
        ScipLanguage::from(lang)
    } else {
        detect_language(&project_path)
    };

    if detected_language == ScipLanguage::Unknown {
        bail!("Could not detect project language. Use --language to specify explicitly.");
    }

    println!("Detected language: {:?}", detected_language);

    // Для C++ проверяем или генерируем compile_commands.json
    let mut compile_commands_path = None;
    if detected_language == ScipLanguage::Cpp {
        if let Some(compdb_path) = compdb {
            compile_commands_path = Some(PathBuf::from(compdb_path));
            if !compile_commands_path.as_ref().unwrap().exists() {
                bail!("compile_commands.json not found: {}", compdb_path);
            }
        } else {
            // Пытаемся сгенерировать автоматически
            println!("No compile_commands.json provided, attempting to generate...");
            
            let compdb_output = project_path.join("compile_commands.json");
            match symgraph_discovery::generate_compile_commands(&project_path, &compdb_output, None) {
                Ok(path) => {
                    println!("Generated compile_commands.json: {}", path.display());
                    compile_commands_path = Some(path);
                }
                Err(e) => {
                    bail!("Failed to generate compile_commands.json: {}. Use --compdb to specify manually.", e);
                }
            }
        }
    }

    // Определяем путь для SCIP файла
    let scip_file = if let Some(output) = scip_output {
        PathBuf::from(output)
    } else {
        // Временный файл
        let temp_dir = std::env::temp_dir();
        let filename = format!("symgraph_{}.scip", Uuid::new_v4());
        temp_dir.join(filename)
    };

    // Создаем конфигурацию SCIP
    let mut scip_config = ScipConfig::new(detected_language.clone(), &project_path, &scip_file);
    
    if let Some(ref compdb_path) = compile_commands_path {
        scip_config = scip_config.with_compile_commands(compdb_path);
    }

    // Проверяем доступность SCIP инструмента
    match check_scip_tool_availability(&detected_language) {
        Ok(true) => {
            println!("Using SCIP tool for {:?}", detected_language);
        }
        Ok(false) => {
            let instruction = get_installation_instruction(&detected_language);
            bail!("SCIP tool not found for {:?}. Install with: {}", detected_language, instruction);
        }
        Err(e) => {
            bail!("Error checking SCIP tool availability: {}", e);
        }
    }

    // Генерируем SCIP индекс
    let scip_path = symgraph_discovery::generate_scip_index(&scip_config)?;
    println!("Generated SCIP index: {}", scip_path.display());

    // Парсим SCIP файл и загружаем в базу данных
    use symgraph_core::{parse_scip_file, load_scip_to_database};
    
    let mut db = symgraph_core::Db::open(db_path)?;
    let scip_data = parse_scip_file(&scip_path)?;
    
    let project_name = format!("{}_project", detected_language);
    load_scip_to_database(&mut db, &scip_data, &project_name)?;
    
    println!("Successfully loaded SCIP data into database: {}", db_path);
    println!("Loaded {} documents, {} symbols, {} occurrences", 
             scip_data.documents.len(), 
             scip_data.symbols.len(), 
             scip_data.occurrences.len());
...
/// Generate project annotation for script languages using SCIP.
fn annotate_project(root: &str, db_path: &str) -> Result<()> {
    use anyhow::bail;
    use symgraph_discovery::{ScipLanguage, detect_language};
    use std::path::PathBuf;
    ...
    match detected_language {
        ScipLanguage::Python | ScipLanguage::JavaScript | ScipLanguage::TypeScript => {
            println!("Annotating {} project using SCIP...", detected_language);
            
            // TODO: Реализовать аннотацию на основе SCIP данных
            println!("Note: SCIP-based annotation is not yet implemented.");
            println!("Please use scan-scip to generate SCIP index first.");
            
            let mut db = Db::open(db_path)?;
            let project_id = db.ensure_project(&format!("{}_project", detected_language), root)?;
            
            // Временная базовая аннотация
            db.update_project_annotation(
                &project_id,
                &format!("{} project analyzed with SCIP", detected_language),
                "scip_analysis",
                "{}",
                "[]"
            )?;
            
            println!("Project annotation created for {} project", detected_language);
        }
        ScipLanguage::Unknown => {
            bail!("Cannot detect language for project annotation. Supported: Python, JavaScript, TypeScript");
        }
        _ => {
            bail!("This is not a script project. Use annotate-compiled for {} projects", detected_language);
        }
    }
