...
use clang::{TranslationUnit, Entity, EntityKind};
use serde::Serialize;

fn is_declaration_kind(kind: EntityKind) -> bool {
    matches!(kind, 
        EntityKind::StructDecl | EntityKind::ClassDecl | EntityKind::EnumDecl |
        EntityKind::FieldDecl | EntityKind::FunctionDecl | EntityKind::VarDecl |
        EntityKind::ParmDecl | EntityKind::TypedefDecl | EntityKind::Method |
        EntityKind::Namespace | EntityKind::Constructor | EntityKind::Destructor |
        EntityKind::ClassTemplate | EntityKind::FunctionTemplate | EntityKind::UnionDecl
    )
}

fn is_expression_or_reference_kind(kind: EntityKind) -> bool {
    matches!(kind,
        EntityKind::CallExpr | EntityKind::DeclRefExpr | EntityKind::MemberRefExpr |
        EntityKind::TypeRef | EntityKind::TemplateRef | EntityKind::NamespaceRef |
        EntityKind::MemberRef | EntityKind::UnexposedExpr
    )
}
...
pub fn scan_tu(tu: &TranslationUnit) -> (Vec<Symbol>, Vec<Occurrence>, Vec<(String,String,String)>) {
    ...
    root.visit_children(|entity, _parent| {
        ...
        if is_declaration_kind(kind) {
            ...
            if let Some(loc) = entity.get_location() {
                ...
                let file = file_loc.file.map(|f| f.get_path().display().to_string()).unwrap_or_default();
            ...
            if let Some(target) = entity.get_reference() {
                ...
                if let Some(loc) = entity.get_location() {
                    ...
                    if kind == EntityKind::CallExpr {
                        if let Some(caller) = entity.get_semantic_parent().and_then(|p| usr_to_string(&p)) {
